...
```>")t?@7O3c:KTqu9iPqI=%#Y;A8P47:KUe/@RP$O=&s!1@Rj0L:fK,3```

After decoding from **Base 85**:

```ZEdoaGRGOXBjMTloWDJ4dmRGOXZabDlpWVhObGN3PT0=```

After decoding from **Base 64**:

```dGhhdF9pc19hX2xvdF9vZl9iYXNlcw==```
...
After decoding from **Decimal to ASCII**:
...
```64 65 66 65 6e 73 65 5f 74 68 65 5f 62 61 73 65```
...
```66L)X@N]DpB6&*8CLnW-De*9fAKWC:AU-<ZFCB'"D^b```

After decoding from **Base 85**:

```B lhb jbgrtske podaee (key: tango)```

After decrypting using **Vigenere** with **tango** as key:
...
```%_+2$tu+'uy!$=q)}u=|&<H`&`5u(=<h```

After decrypting using **ROT 47**:

```T0ZaSEFZVFJPSlBXNFlMUkw1U1dFWlk9```

After decoding from **Base 64**:

```OFZHAYTROJPW4YLRL5SWEZY=```
...
After removing from **(key: tangodown)**:

```GU4AmGDQNTu5ThnmDafNqiwt9yD7QB617C2uu```

After decoding from **Base 58**:

```roh gfh o nrtl purh qnnvkrx```

After decrypting using **Vigenere** with **tangodown** as key:
